...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_dish') {
    ...
    if (!empty($name) && !empty($category) && $price > 0) {
        $update_sql = "UPDATE Dish SET Name = ?, Category = ?, Price = ? WHERE ID = ?";
        $stmt = $conn->prepare($update_sql);
        $stmt->bind_param("ssdi", $name, $category, $price, $id);

        if ($stmt->execute()) {
            logAction($conn, "Редактирование блюда (AJAX)", "ID: $id, Новое название: $name, Новая категория: $category, Новая цена: $price");
        ...
        $response['message'] = 'Заполните все поля корректно. Цена должна быть больше 0.';
    }
...
// Обработка обычной формы (не AJAX)
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_dish'])) {
    ...
    if (!empty($name) && !empty($category) && $price > 0) {
        ...
        if ($stmt->execute()) {
            // Логируем изменение
            logAction($conn, "Редактирование блюда", "ID: $id, Новое название: $name, Новая категория: $category, Новая цена: $price");
            echo "<script>alert('Данные блюда успешно обновлены!'); window.location.href='dishes.php';</script>";
            exit();
        } else {
            $error_message = "Ошибка: Не удалось обновить блюдо. " . $stmt->error;
            logAction($conn, "Ошибка редактирования блюда", "ID: $id, Ошибка: " . $stmt->error);
        }
        $stmt->close();
    } else {
        $error_message = "Ошибка: Заполните все поля корректно. Цена должна быть больше 0.";
    }
}
...
<head>
    ...
    <title>Редактировать блюдо | Панель админа</title>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="content">
            <div class="page-header">
                <h1 class="page-title">Редактирование блюда ID: <?php echo $id; ?></h1>
            ...
            <div class="card">
                ...
                <div class="dish-info">
                    <div class="dish-header">
                        ...
                        <div class="dish-details">
                            ...
                            <div class="dish-meta">
                                <span>Категория: <?php echo htmlspecialchars($dish['Category']); ?></span>
                ...
                <div class="form-section">
                    ...
                    <form id="editDishForm" class="dish-form">
                        <div class="form-group">
                            <label class="form-label" for="dish-id">ID (нельзя изменить)</label>
                            <input type="number" id="dish-id" value="<?php echo $dish['ID']; ?>" readonly class="form-input">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="dish-name">Название блюда</label>
                            <input type="text" id="dish-name" name="name" value="<?php echo htmlspecialchars($dish['Name']); ?>" required class="form-input" placeholder="Введите название блюда">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="dish-category">Категория</label>
                            <input type="text" id="dish-category" name="category" value="<?php echo htmlspecialchars($dish['Category']); ?>" required class="form-input" placeholder="Введите категорию">
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="dish-price">Цена (₽)</label>
                            <input type="number" id="dish-price" name="price" value="<?php echo $dish['Price']; ?>" required min="0.01" step="0.01" class="form-input" placeholder="0.00">
                        </div>
                        <button type="submit" class="btn-save" id="saveDishBtn">
                            <span class="material-icons">save</span>
                            Сохранить изменения
                        </button>
                    </form>
                    <div id="editDishResult"></div>
                </div>
            </div>

            <div class="info-panel">
                <h3 class="info-title">Информация о редактировании</h3>
                <p class="info-text">Редактируйте данные текущего блюда:</p>
                ...
                <ul class="info-list">
                    ...
                    <li class="info-item">
                        <strong>🍽️ Название и Категория</strong>
                        <span>Введите новые данные в соответствующие поля</span>
                    </li>
                    <li class="info-item">
                        <strong>💰 Цена</strong>
                        <span>Укажите новую цену с двумя знаками после запятой</span>
                    </li>
                    <li class="info-item">
                    ...
                    <li class="info-item">
                        ...
                        <span>Вернитесь к списку блюд без сохранения</span>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div id="successModal" class="modal modal-success">
        <div class="modal-content">
            ...
            <p class="modal-description">Данные блюда успешно обновлены. Вы будете перенаправлены к списку через 2 секунды.</p>
            <div class="modal-actions">
                <button class="btn-modal btn-confirm" onclick="closeModal('successModal'); window.location.href='dishes.php';">OK</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const resultDiv = document.getElementById('editDishResult');
        ...
        editForm.addEventListener('submit', async (e) => {
            ...
            formData.append('action', 'update_dish');
